...
The average daily return of SBUX over the last 3-ish year is `r round(r_bar, 5)` and using $\bar{R} = \mu \delta t$ , or $\mu = \frac{\bar{R}}{\delta t}$ and in our case $\delta t = \frac{1}{252}$; hence annualized returns is  `r round(mu, 5) * 100` % with standard deviation of `r round(sigma, 5) * 100` % annualized. 
