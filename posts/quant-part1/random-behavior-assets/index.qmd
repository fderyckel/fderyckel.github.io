...
# The probabilty of Brownian motions

Under a discrete environment, asset's returns are thought as random.  The easiest way to model this is to consider a **symmetric random walk** - a head/tail type of example. 
...
Let's now define another Discrete Random Variable $S_i$ which is the sum of the returns after n toss.  Hence, $\sum_{i=1}^{n} R_i$ with an initial condition that $S_0=0$
...
$$\mathbb{V}ar[S_i] = \mathbb{E} \left[ \left( \sum_{i=0}^{n} R_i \right)^2 \right] = \mathbb{E} \left[ R_0^2 + R_1^2 + \dots + R_n^2 + 2R_1R_2 + 2R_1R_3 + \cdots \right]$$ 

Because the events are independents, $\mathbb{E}[R_i R_j] = 0$.  Hence all the double terms in R above are equals to 0 and 
$$\mathbb{V}ar[S_i] = \mathbb{E}[R_0^2] + \mathbb{E}[R_1^2] + \dots + \mathbb{E}[R_n^2] = 1 + 1 + \dots + 1 = n \cdot 1 = n$$
