...
Principal Component Analysis is a widely method to reduce the dimensionality of a dataset as well as to de-correlate it.  It can also be used to weight the importance of variables.  The PCA transforms variables into another set of variables called *Principal Components*.  

It takes the data and tries to find a direction (let's say vector 1) such that variance of points projected on vector 1 is maximum.  
...
$$\frac{1}{n-1} \left( \textbf{X} - \textbf{\bar{X}} \right)^T \cdot \left( \textbf{X} - \textbf{\bar{X}} \right)$$
...
$$\textbf{A} v = \textbf{\lambda} v$$
$$\left( \textbf{A} - \textbf{\lambda} \right) v = 0$$ 
$$det\left( \textbf{A} - \textbf{\lambda} \textbf{I} \right) = 0$$
